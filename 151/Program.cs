...
namespace _151 {
    class Program {
        static void Main(string[] args) {
            ...
            var maxPrice = Library.Books.Max(item => item.Price);
            var mostExpensiveBook = Library.Books.Where(item => item.Price == maxPrice).FirstOrDefault();
            Console.WriteLine("Most expensive book={0}:{1}:{2}", mostExpensiveBook.Title, mostExpensiveBook.PublishedYear, mostExpensiveBook.Price);

            Console.WriteLine("===============================");

            foreach (var group in Library.Books.GroupBy(item => item.PublishedYear).OrderBy(item => item.Key)) {
            ...
            foreach (var item in Library.Books.OrderBy(item => item.PublishedYear).ThenBy(item => item.Price)) {
                Console.WriteLine("{0}年　{1}円　{2}", item.PublishedYear, item.Price, item.Title);
            }

            Console.WriteLine("===============================");

            foreach (var group in Library.Books.Where(item => item.PublishedYear == 2016).GroupBy(item => item.CategoryId)) {
                Console.WriteLine("{0}:{1}", group.Key, Library.Categories.Where(item => item.Id == group.Key).FirstOrDefault().Name);
            }
            Console.WriteLine("===============================");
            ...
            var query = Library.Books
                .Join(Library.Categories,
                    ...
                    (book, category) => new { category = category.Name, book = book.Title }
                    ).GroupBy(item => item.category).OrderBy(item => item.Key);

            foreach (var group in query) {
                Console.WriteLine(group.Key);
                foreach (var record in group) {
                    Console.WriteLine("    " + record.book);
                }
            }
            Console.WriteLine("===============================");

            var developmentCategory = Library.Categories.Where(item => item.Name == "Development").FirstOrDefault();
            if (developmentCategory == null) {
                throw new Exception("Development category does not found in Library.");
            }
            var query2 = Library.Books.Where(item => item.CategoryId == developmentCategory.Id).GroupBy(item => item.PublishedYear).OrderBy(group => group.Key);
            ...
            Console.WriteLine("===============================");
            var query3 = Library.Categories
                .GroupJoin(Library.Books,
                     category => category.Id,
                     book => book.CategoryId,
                     (category, books) => new { category = category.Name, books = books }
                     ).Where(item => item.books.Count() >= 4);
            foreach (var group in query3) {
                Console.WriteLine("4冊以上カテゴリ:{0}", group.category);
            }
        }
    }
